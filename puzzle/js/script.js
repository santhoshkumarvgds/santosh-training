...
$(document).ready(function () {
    setTimeout(function () {
        ...
        $("button").click(function () {
            ...
            score += 1;
            for (var i = 1; i <= 9; i++) {
                arrcheck[arrpos[i]] = $("#" + i).offset();
            }
            for (var i = 9, j = 1; i >= 1; i--, j++) {
                var mul = i + 10;
                var tdcheck = $("#" + mul).offset();
                //  alert(tdcheck.left +" "+arrcheck[i].left);
                //console.log(checkimg+" "+pos[i-1]);
                if ((tdcheck.left >= (arrcheck[i].left - 8) && tdcheck.left <= (arrcheck[i].left + 8)) && (tdcheck.top >= (arrcheck[i].top - 8) && tdcheck.top <= (arrcheck[i].top + 8))) {
            ...
            } else if ((9 - countallclick) <= 9) {
                alert("Please fill " + (9 - countallclick) + " box or check their is set with correct position");
            } else {
