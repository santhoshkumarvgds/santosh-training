...
import { UserContext } from "../Containers/Context/Context";
...
export default function ViewProduct(props) {
    ...
    const getProduct= async()=>{
      ...
      const response = await fetch(
        "http://localhost:4000/user/getproduct",
        {
          method: "post",
          credentials: "include",
          headers: {
   ...
   <div  className="single-product">
          <h1>{name}</h1><br/>
                ...
                {loginStatus? role == "Admin" ? (
                  <React.Fragment>
                    <a title="click to open seller mail" className="mail" href={"mailto:" + email}>seller : {email}</a>
